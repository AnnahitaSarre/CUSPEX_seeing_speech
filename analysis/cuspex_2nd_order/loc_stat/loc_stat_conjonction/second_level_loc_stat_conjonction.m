...
% Group to analyse
group = 'g1^g2^g3'; % g1^g2 / g2^g3 / g1^g3 / g1^g2+g3 / g3^g1+g2 / g1^g2^g3
...
elseif strcmp(group,'g1^g2+g3')
    subjects_list = [{subjects_g1}, {[subjects_g2, subjects_g3]}];
elseif strcmp(group,'g3^g1+g2')
...
if ~isempty(covariates_list)
    % Retrieve the index of the column where 'id' is stored
...
for i_contrast=1:numel(contrasts_list)
    ...
    matlabbatch_second_level_loc_stat_conjonction_job = second_level_loc_stat_conjonction_job(subjects_list, group, smoothing_level, contrast, covariates);
    spm_jobman('run', matlabbatch_second_level_loc_stat_conjonction_job);
end