...
function matlabbatch = second_level_loc_stat_1g_t_test_job(subjects_list, group, smoothing_level, contrast, covariates)
    ...
    for subject=1:numel(subjects_list)
       scans{subject} = sprintf('/mnt/ata-ST4000NM0165_ZAD8CWEG-part1/cuspex/cuspex_analysis/cuspex_final_images/CUSPEX_s%02d/func/loc_stat/stats/%s/con_00%02d.nii,1', subjects_list(subject), smoothing_dir, contrast_file_num);
    end
    scans = scans.';
    ...
    if ~isempty(covariates)
        for i_covariate=1:numel(covariates{1})
            matlabbatch{3}.spm.stats.con.consess{1+i_covariate}.tcon.name = char(strcat(contrast_name, " pos ", covariates{1}{i_covariate}));
            matlabbatch{3}.spm.stats.con.consess{1+i_covariate}.tcon.weights = [zeros(1,i_covariate), 1];
